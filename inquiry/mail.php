...
//$to = "whitekanegon@gmail.com";
$to = "info@joysmile-kyousei.com";

//フォームのメールアドレス入力箇所のname属性の値（name="○○"　の○○部分）
$Email = "email";

//メールアドレス確認箇所のname属性の値（name="○○"　の○○部分）
...
$Referer_check_domain = "joysmile-kyousei.com/";
...
$thanksPage = "http://joysmile-kyousei.com/inquiry/thanks.html";
...
$require = array('お名前','フリガナ','性別','生年月日','矯正パンフレット','email');
...
$refrom_name = "ジョイスマイル矯正歯科";

// 差出人に送信確認メールを送る場合のメールのタイトル（上記で1を設定した場合のみ）
$re_subject = "お問い合わせを承りました";
...
大阪府豊中市新千里東町1-4-2
TEL：06-6836-3456
E-mail: info@joysmile-kyousei.com
URL: http://joysmile-kyousei.com/
──────────────────────
...
<?php include_once(dirname(__FILE__).'/confirm.php'); ?>
...
//確認画面の入力内容出力用関数
function confirmOutput($arr,$Email_confirm){
	global $hankaku,$hankaku_array;
	$html = '';
	foreach($arr as $key => $val) {
		if($key == $Email_confirm) continue;
		$out = '';
		if(is_array($val)){
			foreach($val as $key02 => $item){ 
				//連結項目の処理
